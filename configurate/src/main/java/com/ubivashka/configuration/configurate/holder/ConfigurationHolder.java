...
public class ConfigurationHolder implements ConfigurationSectionHolder {
    ...
    public <L> List<L> getList(String key) {
        return (List<L>) configurationNode.node(key).childrenList().stream().map(ConfigurationNode::raw).collect(Collectors.toList());
    }
    ...
    public Set<String> getKeys() {
        return configurationNode.childrenMap().keySet().stream().map(object -> String.valueOf(object)).collect(Collectors.toSet());
    }
